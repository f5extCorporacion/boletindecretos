...
// Tipado para los elementos del array
...
const SignIn = () => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const rets = loginUser(user.email , user.password)    
 rets.then((data)=>{
  if(data.mensaje === "Login exitoso"){
    Swal.fire({
      title: `Bienvenido ${JSON.stringify(data.mensaje) }`,
      draggable: true
    });
      window.speechSynthesis.speak(new SpeechSynthesisUtterance('Bienvenido a boletines'));
    window.location.href = '/';
  }
  else{
    Swal.fire({
      title: `Error en email o password `,
      icon: "error",
      draggable: true
    });
    const utterance = new SpeechSynthesisUtterance('Revisa tu email o password');
  ...
  return (
    ...
    <Stack mx="auto" direction="column" alignItems="center" width={1} maxWidth={450}>
      <ButtonBase LinkComponent={Link} href="/" sx={{ mt: 6 }} disableRipple>
        <Image src={Logo} alt="logo" height={100} width={140} />
      </ButtonBase>
      <Divider sx={{ my: 4.5 }}>Login Boletines</Divider>
